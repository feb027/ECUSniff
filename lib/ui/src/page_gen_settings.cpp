...
void PageGenSettings::render(bool fullRedraw, uint8_t editRow,
    ...
    if (fullRedraw) {
        ...
        _gfx->fillRoundRect(16, 48, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("1. RPM STEP (ENCODER):", 24, 53);

        _gfx->fillRoundRect(244, 48, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("2. SWEEP MIN RPM:", 252, 53);

        _gfx->fillRoundRect(16, 114, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("3. SWEEP MAX RPM:", 24, 119);

        _gfx->fillRoundRect(244, 114, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("4. SWEEP RATE (SPEED):", 252, 119);

        _gfx->fillRoundRect(16, 180, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("5. CRANK START RPM:", 24, 185);

        _gfx->fillRoundRect(244, 180, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("6. CRANK DURATION:", 252, 185);

        _gfx->fillRoundRect(16, 246, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("7. CRANK TRANSITION:", 24, 251);

        _gfx->fillRoundRect(244, 246, 220, 60, 6, 0x0841);
        _gfx->setTextColor(TFT_WHITE, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("8. CKP OUTPUT POLARITY:", 252, 251);
    ...
    if (stepVal != _lastRpmStep || fullRedraw) {
        ...
        _gfx->drawString(buf, 24, 69);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Kenaikan per step knob", 24, 91);
    ...
    if (state.sweep.minRpm != _lastSweepMin || fullRedraw) {
        ...
        _gfx->drawString(buf, 252, 69);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Batas Bawah Auto Sweep", 252, 91);
    ...
    if (state.sweep.maxRpm != _lastSweepMax || fullRedraw) {
        ...
        _gfx->setTextColor(0x07E0, 0x0841); _gfx->setTextSize(2);
        _gfx->drawString(buf, 24, 135);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Batas Atas Auto Sweep", 24, 157);
    ...
    if (state.sweep.sweepRateRpmPerSec != _lastSweepRate || fullRedraw) {
        ...
        _gfx->setTextColor(0x07E0, 0x0841); _gfx->setTextSize(2);
        _gfx->drawString(buf, 252, 135);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Kecepatan Sapuan RPM", 252, 157);
    ...
    if (state.cranking.crankingRpm != _lastCrankRpm || fullRedraw) {
        ...
        _gfx->setTextColor(0x07E0, 0x0841); _gfx->setTextSize(2);
        _gfx->drawString(buf, 24, 201);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("RPM Awal Starter Mesin", 24, 223);
    ...
    if (state.cranking.crankDurationMs != _lastCrankDur || fullRedraw) {
        ...
        _gfx->setTextColor(0x07E0, 0x0841); _gfx->setTextSize(2);
        _gfx->drawString(buf, 252, 201);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Durasi Starter Mesin", 252, 223);
        _lastCrankDur = state.cranking.crankDurationMs;
    }

    if (state.cranking.fastFlare != _lastFastFlare || fullRedraw) {
        const char* tStr = state.cranking.fastFlare ? "MELESAT (0.5s)" : "GRADUAL (2.5s)";
        _gfx->setTextColor(0x07E0, 0x0841); _gfx->setTextSize(2);
        _gfx->drawString(tStr, 24, 267);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Kurva Transisi Nyala", 24, 289);
    ...
    if (wheel.inverted != _lastInverted || fullRedraw) {
        ...
        _gfx->drawString(pStr, 252, 267);
        _gfx->setTextColor(0x07FF, 0x0841); _gfx->setTextSize(1);
        _gfx->drawString("Logika Polaritas CKP", 252, 289);
        _lastInverted = wheel.inverted;
    }

    if (editRow != _lastEditRow || fullRedraw) {
        _drawPanel(16, 48, 220, 60, editRow == 0);
        _drawPanel(244, 48, 220, 60, editRow == 1);
        _drawPanel(16, 114, 220, 60, editRow == 2);
        _drawPanel(244, 114, 220, 60, editRow == 3);
        _drawPanel(16, 180, 220, 60, editRow == 4);
        _drawPanel(244, 180, 220, 60, editRow == 5);
        _drawPanel(16, 246, 220, 60, editRow == 6);
        _drawPanel(244, 246, 220, 60, editRow == 7);
...
void PageGenSettings::onEncoderTurn(int32_t delta, uint8_t editRow,
    ...
    if (editRow == 0) {
        ...
        state.cranking.rampDurationMs = state.cranking.fastFlare ? 500 : 2500;
